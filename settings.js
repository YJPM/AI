...
export const PACE_PROMPTS = {
    normal: `
你是我的AI叙事导演。请根据最近对话，生成4条独立的行动选项。
...
`.trim(),
    ...
    fast: `
你是我的AI叙事导演。请根据最近对话，生成4条独立的快速推进或时间转换的行动选项。
...
- 选项必须严格使用"我"的第一人称视角，只能描述我个人的具体行动或对话，禁止描述其他人的动作、对话、心理、反应等。
- 选项必须体现时间快速推进或场景转换，如"几小时后"、"第二天"、"换了个地方"等。
- 选项可以是当前事件的快速完成，也可以是跳跃到新的时间点或场景。
- 每个选项都应该是独立的、不连续的，代表不同的时间推进方向或场景转换选择。
- 选项之间不应该有逻辑上的先后顺序或因果关系。
- 每个选项都应推动剧情或角色关系发展，但方向可以不同。
- 每个选项30字到50字左右。
...
【重要说明】
- 这些选项是平行的、可选择的，不是连续的剧情片段。
- 每个选项代表一个独立的时间推进或场景转换选择。
- 选项之间不应该有"然后"、"接着"、"随后"等表示时间顺序的词汇。
- 避免生成类似"我先...然后...接着...最后..."的连续剧情。
- 每个选项都应该是一个完整的、独立的时间推进或场景转换描述。
